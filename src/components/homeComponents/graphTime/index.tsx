...
import {
  ...
  useGetIndicatorsTimeScholarship,
} from 'src/services/indicadores';
import { Loading } from 'src/components/Loading';

export function GraphTimeScholarship({ year }) {
  ...
  const getDataSeries = () => {
    ...
    if (indicators?.data?.length > 0) {
      indicators.data?.forEach((indicator) => {
        if (indicator.averageMonthsOfTerms > 0) {
          empty = false;
        }
        if (regional) {
          series.push({
            name: indicator.city,
            y: indicator.averageMonthsOfTerms,
  ...
  const options = {
    ...
    tooltip: {
      ...
      useHTML: true,
      headerFormat:
        '<table><tr><th style="text-align: center" colspan="2">Média de Permanência</th></tr>',

      pointFormat:
        '<td><br/></td>' +
        '<tr style="border-top: 0.619403px dashed #7C7C7C;border-bottom: 0.619403px dashed #7C7C7C;"><td>{point.name}:</td>' +
        '<td style="text-align: right"><b>{point.y} Meses</b></td></tr>' +
        '<th style="text-align: center;font-size: 11px;font-weight:400;"><br>CÁLCULO: (data de término ou data <br> de cancelamento do termo - data de inicio) bolsista 1 <br> + (data de término ou data de cancelamento do <br> termo - data de inicio) bolsista 2 / qtd. de bolsistas.</th>',
      footerFormat: '</table>',
      valueDecimals: 0,
    },
    plotOptions: {
