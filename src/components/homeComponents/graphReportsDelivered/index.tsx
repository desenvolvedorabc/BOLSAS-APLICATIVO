...
import { useGetIndicatorsDeliveryReports } from 'src/services/indicadores';
import { Loading } from 'src/components/Loading';

export function GraphReportsDelivered({ year, month }) {
  ...
  const { data: indicators, isLoading: isLoadingIndicators } =
    useGetIndicatorsDeliveryReports(
      {
        year: year,
        month: month?.id,
  ...
  const getDataSeries = () => {
    ...
    if (indicators?.data?.length > 0) {
      indicators.data?.forEach((indicator) => {
        if (indicator.deliveryAverageMonthReports > 0) {
          empty = false;
        }
        if (regional) {
          series.push({
            name: indicator.city,
            y: Number(indicator.deliveryAverageMonthReports),
  ...
  const options = {
    ...
    tooltip: {
      ...
      useHTML: true,
      headerFormat:
        '<table><tr><th style="text-align: center" colspan="2">Taxa de Entrega</th></tr>',
      pointFormat:
        '<td><br/></td>' +
