...
export function TableUsersStates({ url }) {
  ...
  async function loadUsuarios() {
    ...
    resp.data.items?.map((x) => {
      list.push(
        createData(x.id, x.name, x.email, x?.partner_state?.name, x.active),
      );
    });
  ...
  const setRow = (row, index) => {
    ...
    return (
      <Link href={`/usuario-estado/${row.id}`} key={row.id} passHref>
  ...
  const downloadCsv = async () => {
    ...
    saveAs(resp?.data, 'Usuários do Estado');
  };
      ...
      <Container>
        <TopContainer>
          ...
          <div className="d-flex">
            ...
            <Link href="/usuario-estado" passHref>
              <div style={{ width: 160 }}>
                <ButtonPadrao
                  onClick={() => {
